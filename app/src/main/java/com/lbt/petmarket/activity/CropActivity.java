...
//        Bitmap bitmap = ImageController.decodeSampledBitmapFromFile(path);
        ...
        if(bitmap!=null){
            ExifInterface ei;
            try {
                ei = new ExifInterface(ImageController.getImageAbsolutePath(this, uri));
//                ei = new ExifInterface(uri.getPath());
                ...
                switch (orientation) {
                    case ExifInterface.ORIENTATION_ROTATE_90:
                        bitmap = ImageController.RotateBitmap(bitmap, 90);
                        break;
                    case ExifInterface.ORIENTATION_ROTATE_180:
                        bitmap= ImageController.RotateBitmap(bitmap, 180);
                        break;
                    case ExifInterface.ORIENTATION_ROTATE_270:
                        bitmap = ImageController.RotateBitmap(bitmap, 270);
                        break;
                    // etc.
                }
            } catch (IOException e) {
