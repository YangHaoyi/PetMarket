...
public class ImageCacheManager {
	...
	public void init(Context context, String uniqueName, int cacheSize, CompressFormat compressFormat, int quality, CacheType type){
		switch (type) {
		case DISK:
			mDiskDir = getDiskCacheDir(context, uniqueName );
...
//			mImageCache = new BitmapLruImageCache(cacheSize);
		...
		mImageLoader = new ImageLoader(AppController.getApp().getRequestQueue(), mImageCache);
	}
	...
	public void clearCache( ) {
		...
		final File[] files = mDiskDir.listFiles();
		if(files!=null){
